...
class VizManager:
    ...
    def plot_bar(self, df, title, save = False, name = None, desc = None):
        
        plt.bar(df.iloc[:,1],df.iloc[:,2])
        plt.title(title)
        plt.xlabel(df.columns.to_list()[0])
        plt.ylabel(df.columns.to_list()[1])
        plt.show()
        if (save):
            self.load_tracker()

            plt.savefig(self.charts_dir+"/"+name)
